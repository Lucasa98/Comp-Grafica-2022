...
glm::vec3 warpPoint(const Delaunay &delaunay0, const Delaunay &delaunay1, glm::vec3 p) {
	/// @todo: completar
	/*
		dalaunay0 tiene los puntos y triangulos originales
		dalaunay1 tiene los puntos y triangulos desplazados
		p es la coordenada de un vectice deformado
		DEVUELVE la posicion nueva del punto (posicion deformada)
	*/
	
	//Obtener el triangulo al que pertenece p (de la triangulacion original)
	int iT = delaunay0.enQueTriangulo(p);					//indice del triangulo
